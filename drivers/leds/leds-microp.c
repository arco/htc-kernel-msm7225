...
static ssize_t microp_led_breath_times_store(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	struct led_classdev *led_cdev;
	struct microp_led_data *ldata;
	uint8_t data[3] = {0x05, 0xFF, 0xFF};
	int val, ret;
	...
	data[1] = ((val >> 8) & 0x000000FF);
	data[2] = val & 0x000000FF;
	pr_info("%s, data[1] = 0x%x, data[2] = 0x%x\n", __func__, data[1], data[2]);

	ret = microp_i2c_write(MICROP_I2C_WCMD_JOGBALL_LED_MODE, data, 3);
	if (ret)
		pr_err("%s:%s set breath_times failed\n", __func__, led_cdev->name);
...
static int microp_led_probe(struct platform_device *pdev)
{
	...
	if (pdata->special_led_setting != NULL)
		pdata->special_led_setting();
	...
	for (i = 0; i < pdata->num_leds; i++) {
		...
		ret = device_create_file(ldata[i].ldev.dev, &dev_attr_breath_times);
		if (ret < 0) {
