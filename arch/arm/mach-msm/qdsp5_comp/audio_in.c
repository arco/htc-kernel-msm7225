/* arch/arm/mach-msm/qdsp5_comp/audio_in.c
 *
		...
		char __user *buf, size_t count, loff_t *pos)
{
	...
	while (count > 0) {
		...
		if (count >= size) {
			...
			MM_AUD_ERR("audio_in: short read\n");
			break;
		}
	}
	mutex_unlock(&audio->read_lock);
...
static long audpre_ioctl(struct file *file, unsigned int cmd,
		unsigned long arg)
{
	struct audio_in *audio = file->private_data;
	int rc = 0, enable;
...
static int __init audio_in_init(void)
{
	...
	if (!the_audio_in.data) {
		printk(KERN_ERR "[AUD] %s: Unable to allocate DMA buffer\n",
			__func__);
		return -ENOMEM;
	}
