/* arch/arm/mach-msm/board-marvelct.c
...
void config_marvelct_camera_on_gpios(void);
void config_marvelct_camera_off_gpios(void);


#ifdef CONFIG_MICROP_COMMON
void __init marvelct_microp_init(void);
#endif

/* HTC_HEADSET_GPIO Driver */
static struct htc_headset_gpio_platform_data htc_headset_gpio_data = {
	.hpin_gpio		= MARVELCT_GPIO_35MM_HEADSET_DET,
	.key_enable_gpio	= 0,
	.mic_select_gpio	= 0,
};
...
static struct microp_i2c_platform_data microp_data = {
	...
	.gpio_reset = MARVELCT_GPIO_UP_RESET_N,
	.spi_devices = SPI_GSENSOR,
};

static int marvelct_ts_cy8c_wake(void)
{
	printk(KERN_INFO "%s():\n", __func__);
	gpio_direction_output(MARVELCT_GPIO_TP_ATT_N, 0);
	udelay(500);
	gpio_direction_input(MARVELCT_GPIO_TP_ATT_N);

	return 0;
}

static int marvelct_ts_cy8c_power(int on)
{
	printk(KERN_INFO "%s():\n", __func__);
	if (on) {
		gpio_set_value(MARVELCT_V_TP_3V3_EN, 1);
		msleep(2);
		gpio_set_value(MARVELCT_GPIO_TP_RST_N, 0);
		msleep(2);
		gpio_set_value(MARVELCT_GPIO_TP_RST_N, 1);
	}
	return 0;
}

struct cy8c_i2c_platform_data marvelct_ts_cy8c_data[] = {
	{
		.version = 0x0D,
		.abs_x_min = 0,
		.abs_x_max = 1023,
		...
		.abs_width_min = 0,
		.abs_width_max = 512,
		.power = marvelct_ts_cy8c_power,
		.wake = marvelct_ts_cy8c_wake,
		.gpio_irq = MARVELCT_GPIO_TP_ATT_N,
	},
	{
		.version = 0x00,
		.abs_x_min = 0,
...
static void marvelct_phy_reset(void)
{
	int ret;
...
static uint32_t usb_ID_PIN_ouput_table[] = {
	PCOM_GPIO_CFG(MARVELCT_GPIO_USB_ID_PIN, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_4MA),
};

void config_marvelct_usb_id_gpios(bool output)
{
	if (output) {
		config_gpio_table(usb_ID_PIN_ouput_table,
			ARRAY_SIZE(usb_ID_PIN_ouput_table));
		gpio_set_value(MARVELCT_GPIO_USB_ID_PIN, 1);
	} else
...
static struct cm3628_platform_data cm3628_pdata = {
	.intr = MARVELCT_GPIO_PROXIMITY_INT,
...
static struct i2c_board_info i2c_devices[] = {
	{
		...
		.platform_data = &marvelct_ts_cy8c_data,
		.irq = MSM_GPIO_TO_INT(MARVELCT_GPIO_TP_ATT_N)
	},
	{
		I2C_BOARD_INFO(MICROP_I2C_NAME, 0xCC >> 1),
		.platform_data = &microp_data,
		.irq = MARVELCT_GPIO_TO_INT(MARVELCT_GPIO_UP_INT_N)
	},
	{
	...
	},
	{
		...
		.irq = MSM_GPIO_TO_INT(MARVELCT_GPIO_PROXIMITY_INT),
	},
	{
		I2C_BOARD_INFO(AKM8975_I2C_NAME, 0x1A >> 1),
		.platform_data = &compass_platform_data,
		.irq = MARVELCT_GPIO_TO_INT(MARVELCT_GPIO_COMPASS_RDY),
	},
};

static struct pwr_sink marvelct_pwrsink_table[] = {
	{
		.id     = PWRSINK_AUDIO,
		.ua_max = 100000,
	},
...
static int marvelct_pwrsink_resume_early(struct platform_device *pdev)
{
	htc_pwrsink_set(PWRSINK_SYSTEM_LOAD, 7);
	return 0;
}

static void marvelct_pwrsink_resume_late(struct early_suspend *h)
{
	htc_pwrsink_set(PWRSINK_SYSTEM_LOAD, 38);
}

static void marvelct_pwrsink_suspend_early(struct early_suspend *h)
{
	htc_pwrsink_set(PWRSINK_SYSTEM_LOAD, 7);
}

static int marvelct_pwrsink_suspend_late(struct platform_device *pdev, pm_message_t state)
{
	htc_pwrsink_set(PWRSINK_SYSTEM_LOAD, 1);
	return 0;
}

static struct pwr_sink_platform_data marvelct_pwrsink_data = {
	.num_sinks      = ARRAY_SIZE(marvelct_pwrsink_table),
	.sinks          = marvelct_pwrsink_table,
	.suspend_late	= marvelct_pwrsink_suspend_late,
	.resume_early	= marvelct_pwrsink_resume_early,
	.suspend_early	= marvelct_pwrsink_suspend_early,
	.resume_late	= marvelct_pwrsink_resume_late,
};

static struct platform_device marvelct_pwr_sink = {
	.name = "htc_pwrsink",
	.id = -1,
	.dev    = {
		.platform_data = &marvelct_pwrsink_data,
	},
};
...
//	printk(KERN_INFO "%s():\n", __func__);
	...
	gpio_request(MARVELCT_GPIO_VCM_PD, "cam_pwr_on");
	gpio_direction_output(MARVELCT_GPIO_VCM_PD, 0);
	gpio_free(MARVELCT_GPIO_VCM_PD);
...
static struct msm_camera_device_platform_data msm_camera_device_data = {
	.camera_gpio_on  = config_marvelct_camera_on_gpios,
	.camera_gpio_off = config_marvelct_camera_off_gpios,
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k4e1gx_data = {
	...
	.sensor_reset   = MARVELCT_GPIO_CAM_RST_N,
	.vcm_pwd        = MARVELCT_GPIO_VCM_PD,
...
static struct platform_device marvelct_rfkill = {
	.name = "marvelct_rfkill",
	.id = -1,
};
...
static uint32_t fl_gpio_table[] = {
	PCOM_GPIO_CFG(MARVELCT_GPIO_FLASHLIGHT, 0,
					GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA),

};

static void config_marvelct_flashlight_gpios(void)
{
	config_gpio_table(fl_gpio_table, ARRAY_SIZE(fl_gpio_table));
}

static struct flashlight_platform_data marvelct_flashlight_data = {
	.gpio_init		= config_marvelct_flashlight_gpios,
	.flash			= MARVELCT_GPIO_FLASHLIGHT,
	.flash_duration_ms	= 600,
	.chip_model		= AAT3177,
};

static struct platform_device marvelct_flashlight_device = {
	.name = FLASHLIGHT_NAME,
	.dev		= {
		.platform_data	= &marvelct_flashlight_data,
	},
};
...
static struct platform_device *devices[] __initdata = {
	...
	&marvelct_pwr_sink,
#endif
	&marvelct_flashlight_device,
};

extern struct sys_timer msm_timer;

static void __init marvelct_init_irq(void)
{
	printk("marvelct_init_irq()\n");
	msm_init_irq();
}
...
static void marvelct_reset(void)
{
	gpio_set_value(MARVELCT_GPIO_MSM_PS_HOLD, 0);
}
...
static uint32_t marvelct_serial_debug_table[] = {
	/* config as serial debug uart */
	PCOM_GPIO_CFG(MARVELCT_GPIO_UART3_RX, 1,
			GPIO_INPUT, GPIO_PULL_UP, GPIO_8MA),	/* UART3 RX */
	PCOM_GPIO_CFG(MARVELCT_GPIO_UART3_TX, 1,
			GPIO_OUTPUT, GPIO_NO_PULL, GPIO_8MA),	/* UART3 TX */
};

static void marvelct_config_serial_debug_gpios(void)
{
	config_gpio_table(marvelct_serial_debug_table,
			ARRAY_SIZE(marvelct_serial_debug_table));
}

static void __init config_gpios(void)
{
	marvelct_config_serial_debug_gpios();
	config_marvelct_camera_off_gpios();
}

static struct msm_acpu_clock_platform_data marvelct_clock_data = {
...
static unsigned marvelct_perf_acpu_table[] = {
	245760000,
	480000000,
	600000000,
};

static struct perflock_platform_data marvelct_perflock_data = {
	.perf_acpu_table = marvelct_perf_acpu_table,
	.table_size = ARRAY_SIZE(marvelct_perf_acpu_table),
};

#ifdef CONFIG_SERIAL_MSM_HS
static struct msm_serial_hs_platform_data msm_uart_dm1_pdata = {
	.rx_wakeup_irq = MSM_GPIO_TO_INT(MARVELCT_GPIO_BT_HOST_WAKE),
	.inject_rx_on_wakeup = 0,
	.cpu_lock_supported = 1,

	/* for bcm */
	.bt_wakeup_pin_supported = 1,
	.bt_wakeup_pin = MARVELCT_GPIO_BT_CHIP_WAKE,
	.host_wakeup_pin = MARVELCT_GPIO_BT_HOST_WAKE,

};
#endif

static ssize_t marvelct_virtual_keys_show(struct kobject *kobj,
...
static struct kobj_attribute marvelct_virtual_keys_attr = {
	.attr = {
		.name = "virtualkeys.cy8c-touchscreen",
		.mode = S_IRUGO,
	},
	.show = &marvelct_virtual_keys_show,
};

static struct attribute *marvelct_properties_attrs[] = {
	&marvelct_virtual_keys_attr.attr,
	NULL
};

static struct attribute_group marvelct_properties_attr_group = {
	.attrs = marvelct_properties_attrs,
};

static void __init marvelct_init(void)
{
	int rc;
	char *cid = NULL;
	struct kobject *properties_kobj;

	printk("marvelct_init() revision = 0x%X\n", system_rev);
	msm_clock_init();
	 ...
	 * vreg.
	 */
	gpio_request(MARVELCT_GPIO_LS_EN, "ls_en");
	gpio_direction_output(MARVELCT_GPIO_LS_EN, 0);


	msm_hw_reset_hook = marvelct_reset;

	msm_acpu_clock_init(&marvelct_clock_data);
	perflock_init(&marvelct_perflock_data);

#if defined(CONFIG_MSM_SERIAL_DEBUGGER)
	if (!opt_disable_uart3)
		msm_serial_debug_init(MSM_UART3_PHYS, INT_UART3,
			&msm_device_uart3.dev, 1,
				MSM_GPIO_TO_INT(MARVELCT_GPIO_UART3_RX));
#endif
	...
	*/
	...
	msm_add_usb_id_pin_gpio(MARVELCT_GPIO_USB_ID_PIN);
	msm_add_usb_devices(marvelct_phy_reset, NULL);
#endif
	...
	config_marvelct_usb_id_gpios(0);
...
#ifdef CONFIG_MICROP_COMMON
	marvelct_microp_init();
#endif

	rc = marvelct_init_mmc(system_rev);
	if (rc)
...
static void __init marvelct_fixup(struct machine_desc *desc, struct tag *tags,
...
static void __init marvelct_map_io(void)
{
	printk("marvelct_init_map_io()\n");
	msm_map_common_io();
	if (socinfo_init() < 0)
		BUG();
#ifdef CONFIG_CACHE_L2X0
	...
	if ((SOCINFO_VERSION_MAJOR(socinfo_get_version()) > 1)
		...
		&& (SOCINFO_VERSION_MINOR(socinfo_get_version()) >= 3)))
		{
			...
			printk("Marvelct_init L2-cache latency 4 cyc for 7x27-t\n");
		}else{
		/* R/W latency: 3 cycles; */
	l2x0_init(MSM_L2CC_BASE, 0x00068012, 0xfe000000);
			printk("Marvelct_init L2-cache latency 3 cyc for 7x27\n");
		}
#endif
}

MACHINE_START(MARVELCT, "marvelct")
#ifdef CONFIG_MSM_DEBUG_UART
	.phys_io        = MSM_DEBUG_UART_PHYS,
	.io_pg_offst    = ((MSM_DEBUG_UART_BASE) >> 18) & 0xfffc,
#endif
	.boot_params    = 0x12C00100,
	.fixup          = marvelct_fixup,
	.map_io         = marvelct_map_io,
	.init_irq       = marvelct_init_irq,
	.init_machine   = marvelct_init,
	.timer          = &msm_timer,
MACHINE_END
