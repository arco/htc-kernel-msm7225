/* linux/arch/arm/mach-msm/board-icong-mmc.c
...
static int __init icong_disablesdcard_setup(char *str)
{
...
__setup("board_icong.disable_sdcard=", icong_disablesdcard_setup);
...
static uint32_t icong_sdslot_switchvdd(struct device *dev, unsigned int vdd)
{
	int i;
...
static unsigned int icong_sdslot_status(struct device *dev)
{
	unsigned int status;

	status = (unsigned int) gpio_get_value(ICONG_GPIO_SDMC_CD_N);
	return (!status);
}

#define ICONG_MMC_VDD		(MMC_VDD_28_29 | MMC_VDD_29_30)

static unsigned int icong_sdslot_type = MMC_TYPE_SD;

static struct mmc_platform_data icong_sdslot_data = {
	.ocr_mask	= ICONG_MMC_VDD,
	.status_irq	= MSM_GPIO_TO_INT(ICONG_GPIO_SDMC_CD_N),
	.status		= icong_sdslot_status,
	.translate_vdd	= icong_sdslot_switchvdd,
	.slot_type	= &icong_sdslot_type,
};
 ...
 */
static struct embedded_sdio_data icong_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int icong_wifi_cd;		/* WIFI virtual 'card detect' status */
static void (*wifi_status_cb)(int card_present, void *dev_id);
static void *wifi_status_cb_devid;

static int
icong_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static unsigned int icong_wifi_status(struct device *dev)
{
	return icong_wifi_cd;
}

static struct mmc_platform_data icong_wifi_data = {
	.ocr_mask		= MMC_VDD_28_29,
	.status			= icong_wifi_status,
	.register_status_notify	= icong_wifi_status_register,
	.embedded_sdio		= &icong_wifi_emb_data,
};

int icong_wifi_set_carddetect(int val)
{
...
EXPORT_SYMBOL(icong_wifi_set_carddetect);

int icong_wifi_power(int on)
{
	int rc = 0;
	...
	mdelay(100);
	gpio_set_value(ICONG_GPIO_WIFI_EN, on);
	mdelay(100);
	return 0;
}
EXPORT_SYMBOL(icong_wifi_power);

int icong_wifi_reset(int on)
{
	printk(KERN_INFO "%s: do nothing\n", __func__);
	return 0;
}

int __init icong_init_mmc(unsigned int sys_rev)
{
	...
	id = PCOM_GPIO_CFG(ICONG_GPIO_WIFI_EN, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),
	msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &id, 0);
	gpio_set_value(ICONG_GPIO_WIFI_EN, 0);

	wifi_status_cb = NULL;

	printk(KERN_INFO "%s\n", __func__);

	msm_add_sdcc(1, &icong_wifi_data, 0, 0);

	register_msm_irq_mask(INT_SDC2_0);
	register_msm_irq_mask(INT_SDC2_1);
	if (opt_disable_sdcard) {
		printk(KERN_INFO "icong: SD-Card interface disabled\n");
		goto done;
	}
	...
	set_irq_wake(MSM_GPIO_TO_INT(ICONG_GPIO_SDMC_CD_N), 1);

	msm_add_sdcc(2, &icong_sdslot_data, MSM_GPIO_TO_INT(ICONG_GPIO_SDMC_CD_N),
