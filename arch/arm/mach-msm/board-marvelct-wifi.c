/* linux/arch/arm/mach-msm/board-marvelct-wifi.c
*/
...
int marvelct_wifi_set_carddetect(int on);
int marvelct_wifi_get_mac_addr(unsigned char *buf);
...
static void *marvelct_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init marvelct_init_wifi_mem(void)
{
	int i;
...
static struct wifi_platform_data marvelct_wifi_control = {
	.set_power      = marvelct_wifi_power,
	.set_reset      = marvelct_wifi_reset,
	.set_carddetect = marvelct_wifi_set_carddetect,
...
static struct platform_device marvelct_wifi_device = {
        .name           = "bcm4329_wlan",
        .id             = 1,
        .num_resources  = ARRAY_SIZE(marvelct_wifi_resources),
        .resource       = marvelct_wifi_resources,
        .dev            = {
                .platform_data = &marvelct_wifi_control,
        },
};

extern unsigned char *get_wifi_nvs_ram(void);

static unsigned marvelct_wifi_update_nvs(char *str)
{
...
int marvelct_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk("marvelct_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	marvelct_wifi_update_nvs("sd_oobonly=1\n");
#endif
	marvelct_wifi_update_nvs("btc_params80=0\n");
	marvelct_wifi_update_nvs("btc_params6=30\n");
	marvelct_init_wifi_mem();
	ret = platform_device_register(&marvelct_wifi_device);
	return ret;
}
