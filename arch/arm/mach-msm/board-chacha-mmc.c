/* linux/arch/arm/mach-msm/board-chacha-mmc.c
...
static int __init chacha_disablesdcard_setup(char *str)
{
...
__setup("board_chacha.disable_sdcard=", chacha_disablesdcard_setup);
...
static unsigned int chacha_sdslot_status(struct device *dev)
{
	unsigned int status;

	status = (unsigned int) gpio_get_value(CHACHA_GPIO_SDMC_CD_N);
	return (!status);
}

#define CHACHA_MMC_VDD		(MMC_VDD_28_29 | MMC_VDD_29_30)

static unsigned int chacha_sdslot_type = MMC_TYPE_SD;

static struct mmc_platform_data chacha_sdslot_data = {
	.ocr_mask	= CHACHA_MMC_VDD,
	.status_irq	= MSM_GPIO_TO_INT(CHACHA_GPIO_SDMC_CD_N),
	.status		= chacha_sdslot_status,
	.translate_vdd	= chacha_sdslot_switchvdd,
	.slot_type	= &chacha_sdslot_type,
};
 ...
 */
static struct embedded_sdio_data chacha_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int chacha_wifi_cd;		/* WIFI virtual 'card detect' status */
static void (*wifi_status_cb)(int card_present, void *dev_id);
static void *wifi_status_cb_devid;

static int
chacha_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static unsigned int chacha_wifi_status(struct device *dev)
{
	return chacha_wifi_cd;
}

static struct mmc_platform_data chacha_wifi_data = {
	.ocr_mask		= MMC_VDD_28_29,
	.status			= chacha_wifi_status,
	.register_status_notify	= chacha_wifi_status_register,
	.embedded_sdio		= &chacha_wifi_emb_data,
};
...
EXPORT_SYMBOL(chacha_wifi_set_carddetect);

int chacha_wifi_power(int on)
{
	int rc = 0;
	...
	mdelay(100);
	gpio_set_value(CHACHA_GPIO_WIFI_EN, on);
	mdelay(100);
	return 0;
}
EXPORT_SYMBOL(chacha_wifi_power);

int chacha_wifi_reset(int on)
{
